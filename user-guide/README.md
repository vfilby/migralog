...
the app. The guide is authored here once and "compiled" into both:

- **Web** — the `/guide/` page on `migralog.app`, generated by
  [`website/build-user-guide.py`](../website/build-user-guide.py) into
  `website/website/guide/index.html` and deployed by the website pipeline.
- **In-app** — bundled into the iOS app (see `mobile-apps/ios/project.yml`) and
  rendered natively by the User Guide views under `Views/Help`.

## Status

The in-app guide and the web build are both live. Edit the Markdown here, then
regenerate the web page:

```bash
python3 website/build-user-guide.py   # writes website/website/guide/index.html
```

The generated `guide/index.html` is committed alongside the rest of the static
site (the deploy step only syncs files, it does not run the generator), so
**re-run the script and commit its output whenever you change the guide.**
