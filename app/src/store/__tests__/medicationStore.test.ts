...
describe('medicationStore', () => {
  ...
  describe('getTodaysMedications', () => {
    it('should return empty array when no preventative medications', () => {
      const result = useMedicationStore.getState().getTodaysMedications();
    ...
    it('should mark medication as taken when dose logged today', () => {
      ...
      expect(result[0].takenAt).toBeDefined();
      expect(result[0].doseId).toBe('dose-1');
    });
  });
});
