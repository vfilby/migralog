...
import { backupService, BackupMetadata } from '../../services/backup/backupService';
...
export default function DataSettingsScreen({ navigation }: Props) {
  ...
  const renderBackupItem = (backup: BackupMetadata) => {
    ...
    if (!backup.id || backup.id === 'undefined') {
      logger.warn('[DataSettingsScreen] Skipping render of backup with invalid ID:', backup);
      return null;
    }
  ...
  return (
    <View style={styles.container} testID="data-settings-screen">
      {/* Header */}
